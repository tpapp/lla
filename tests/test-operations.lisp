...
(addtest (operations-tests)
  ...
  (let ((m (clo 1 4 :/
    ...
    (ensure-same (reshape (clo 1 3 5 :/
                          ...
                          '(3 2) :column-major)
                 m)
    (ensure-same (reshape (clo 1 4 :/ 2 5 3 6) '(3 t) :column-major) m)
    (ensure-same (reshape (clo 1 2 :/ 3 4 5 6) '(t 3) :row-major) 
                 (clo 1 2 3 :/ 4 5 6))
    (ensure-error (reshape m '(1) :column-major))
    (ensure-error (reshape m '(1) :row-major))
