...
(defmethod mm ((a diagonal) (b dense-matrix-like) &optional (alpha 1))
  ...
  (bind (((:slots-read-only (a-lla-type lla-type) (diagonal-elements elements)) a)
         ((:slots-read-only (b-lla-type lla-type) nrow ncol (matrix-elements elements)) b)
         (common-type (common-target-type a-lla-type b-lla-type alpha))
    ...
    (if (and (eq common-type a-lla-type)
             (eq common-type b-lla-type))
        (expand-for-lla-types (common-type :prologue (ecase common-type))
          `(,common-type
            (locally
                (declare (type ,(nv-array-type common-type) matrix-elements diagonal-elements
                               result-elements)
                         (type ,(lla-type->lisp-type common-type) alpha))
              (dotimes (col ncol)
                (dotimes (row nrow)
